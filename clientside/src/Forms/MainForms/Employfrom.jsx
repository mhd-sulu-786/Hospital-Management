...
const EmployeeForm = ({ setActive }) => {
    ...
    useEffect(() => {
        const fetchReportForDepartment = async () => {
            try {
                ...
                if (repo) {
                    setFormData(prevState => ({ ...prevState, report: repo.name }));
                }
            } catch (error) {
    ...
    const submitHandle = async () => {
        ...
        if (Object.keys(validationErrors).length > 0) {
            ...
            formDataToSubmit.append('report', formData.report);
    ...
    return (
        <Container className="d-flex justify-content-center align-items-center text-center flex-column my-5 p-5 col-md-6">
            <Form className="d-flex justify-content-center align-items-center text-center gap-2 flex-column bg-success p-5 w-100" style={{ borderRadius: '10px' }}>
                <h2 className='text-info'>Add a New Employee</h2>
                ...
                <Form.Group controlId="formReport" className="w-100">
                    <Form.Control
                        as="select"
                        name="report"
